...
def notify_topic_subscribers(post):
    ...
    if post != topic.head:
        ...
        for user in topic.subscribers.exclude(pk=post.user.pk):
            notifications = post.topic.notifications.filter(topic=topic,
                                                            user=user)
            if not notifications:
                post.topic.notifications.create(topic=topic, user=user)
                try:
                    validate_email(user.email)
                except:
                    continue
                if (user.email == '%s@example.com' % getattr(
                        user, compat.get_username_field())):
                    continue

                lang = (util.get_pybb_profile(user).language or
